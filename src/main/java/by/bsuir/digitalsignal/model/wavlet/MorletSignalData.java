...
public class MorletSignalData extends GraphSignalData {

    private final int N;
    private final int SDVIG = 0;

    public MorletSignalData(GraphSignalData signalData, float fw, CommonData commonData) {
        super(commonData);
        ...
        this.N = Math.round(8 * this.frequencyResolution * this.sampleSize / fw);
        ...
        for (int i = 0; i < this.data.length; ++i) {
            ...
            this.data[i] = (float) (Math.cos(2 * Math.PI * p) * Math.exp(-(p * p) / 2));
        }
    ...
    @Override
    public MorletSignalData clone() throws CloneNotSupportedException {
        return (MorletSignalData) super.clone();
    }
